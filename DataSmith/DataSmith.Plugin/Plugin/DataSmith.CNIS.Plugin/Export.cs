	...
	public class Export : IDataTransfer
	{
        ...
        public void DataTransfer(JoinContext context)
        {
        	...
        	try {
				...
				switch (flag) {
					case "1":                        
						Console.WriteLine("1.刷新单个住院患者>>>");
						zyh = args[1];
						absImp = new ImpPatientSign(zyh);
						break;
					case "2":                        
						Console.WriteLine("2.批量导住院患者信息>>>");
						absImp = new ImpPatient();
						break;
					case "3":
						Console.WriteLine("3.批量导门诊患者>>>");
						absImp = new ImpOPatient();
						break;
					case "4":
						Console.WriteLine("4.导单个患者检验数据>>>");
						zyh = args[1];
						absImp = new ImpLis(zyh);
						break;
					case "5":
						Console.WriteLine("5.导单个患者会诊>>>");
						zyh = args[1];
						absImp = new ImpConsultation(){zyh = zyh};
						break;
					case "6":
						Console.WriteLine("6.批量导患者会诊>>>");
					...
					case "8":
						...
						break;
					case "9":
						Console.WriteLine("9.增量导检验数据>>>");
						absImp = new ImpLisNear();
						break;
					case "10":                        
						Console.WriteLine("10.批量按入院时间段导住院患者信息>>>");
						absImp = new ImpPatient(){
